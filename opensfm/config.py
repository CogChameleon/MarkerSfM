...
prune_with_metadata: no       # If on, will prune matches by gps, time, sequence, etc...
prune_with_tags: strict       # 'strict','medium','loose','none' - strict will not merge separate components, medium will merge single unconnected images, loose will merge all
...
# Tag params
prune_features_on_tags: no         # removes any featurs that are on the detected tags so that only tag corners are used as features in those regions of images
...
ba_constraint_reproj: no           # Bundle Adjustment constraint for tag point reprojection error
tag_size_sd: 0.01                  # The standard deviation of the tag size
tag_ortho_sd: 0.01                 # The standard deviation of the tag orthogonality
tag_reprojection_error_sd: 0.004   # The standard deviation of the tag reprojection error
tag_loss_function: SoftLOneLoss     # Loss function for the ceres problem (see: http://ceres-solver.org/modeling.html#lossfunction)
tag_loss_function_threshold: 1      # Threshold on the squared residuals.  Usually cost is quadratic for smaller residuals and sub-quadratic above.
ratio_tags_to_keep: 1.0         # 1.0 means all tags are kept, 0.0 means all tags are ignored. This is used for experimentation on density of tags
'''
