...
def point_from_json(key, obj):
    ...
    if "on_tag" in obj:
        point.on_tag = obj["on_tag"]
    if "tag_id" in obj:
        point.tag_id = obj["tag_id"]
    if "tag_corner" in obj:
        point.tag_corner = obj["tag_corner"]
    return point

def tag_detections_from_json(obj):
    """
    Read tag detections from a json object
    """
    ...
    # for each image
    for key, value in obj["detections"].iteritems():
        
        # number of detections
        num_dets = int( value["num_dets"] )
        if num_dets == 0:
            detections[key] = []
            continue

        # create new TagDetection for each det
        tag_detections_list = []
        for tagid, det in value["dets"].iteritems():

            # create new detection
            newTD = types.TagDetection()
            newTD.id = tagid
            newTD.hamming  = int(det["hamming"])
            newTD.goodness = int(det["goodness"])
            newTD.margin   = float(det["margin"])
            newTD.homography = np.array( det["homography"], dtype=np.float )
            newTD.center  = np.array( det["center"], dtype=np.float )
            newTD.corners = np.array( det["corners"], dtype=np.float )
            newTD.colors  = np.array( det["colors"], dtype=np.float )

            # add detection to list
            tag_detections_list.append(newTD)

        # add TagDetection to list
        detections[key] = tag_detections_list

    # return detections
    return detections

def images_with_tag_detections_to_json(tag_detections):
    """
    Write tag detections to a json object
    """
    obj = {
        "detections": {}
    }

    # Extract tag detections for each image
    for image in tag_detections:
        obj['detections'][image] = tag_detections_to_json(tag_detections[image])

    return obj
    
def tag_detections_to_json(tag_detections):
    """
    Write tag detections to json object
    """
    obj = {
        'num_dets': len(tag_detections),
        'dets': {}
    }

    # extract tag detection from tag detections
    for tag_detection in tag_detections:
        obj['dets'][tag_detection.id] = tag_detection_to_json(tag_detection)

    return obj

def tag_detection_to_json(tag_detection):
    """
    Write tag detection to json object
    """

    return {
        'hamming': tag_detection.hamming,
        'goodness': tag_detection.goodness,
        'margin': tag_detection.margin,
        'homography': tag_detection.homography.tolist(),
        'center': list(tag_detection.center),
        'corners': tag_detection.corners.tolist(),
        'colors': tag_detection.colors.tolist()
    }
