...
def bundle(graph, reconstruction, gcp, config):
        ...
        if shot_id in graph:
            for track in graph[shot_id]:
                if track in reconstruction.points:
                    if config.get('tag_tracks',False) and graph[shot_id][track]['tag_feature']:
                        if config.get('optimize_with_tag_tracks',False):
                            ba.add_tag_observation(str(shot_id), str(track), *graph[shot_id][track]['feature'])
                        else:
    ...
    # turn on settings if config is set
    use_tag_constraints = False
    if config.get('ba_constraint_size', False):
        ba.set_tag_size_sd(config.get('tag_size_sd',0.01))
        ba.set_use_tag_size_constraint(True)
        use_tag_constraints = use_tag_constraints or True
    if config.get('ba_constraint_ortho', False):
        ba.set_tag_ortho_sd(config.get('tag_ortho_sd',0.01))
        ba.set_use_tag_ortho_constraint(True)
        use_tag_constraints = use_tag_constraints or True
    if config.get('ba_constraint_reproj', False):
        ba.set_tag_loss_function( config.get('tag_loss_function', 'SoftLOneLoss'), config.get('tag_loss_function_threshold', 1) )
        ba.set_tag_reproj_sd(config.get('tag_reprojection_error_sd',0.004))
        ba.set_use_tag_reproj_constraint(True)

    # if size or ortho are used, add tags to BA
    if use_tag_constraints:
        ...
        for tag, corners in tags.iteritems():
            ...
            if not None in corners:
                ...
                c2 = corners[2]
                c3 = corners[3]
                ba.add_tag(c0[0],c0[1],c0[2],
                           c1[0],c1[1],c1[2],
                           c2[0],c2[1],c2[2],
                           c3[0],c3[1],c3[2],
                           config.get('tag_size',0.1))

    #=== End Tag Constraints ===#

    ba.set_loss_function( config.get('loss_function', 'SoftLOneLoss'), config.get('loss_function_threshold', 1) )
    ba.set_reprojection_error_sd(config.get('reprojection_error_sd', 0.004))
    ba.set_internal_parameters_prior_sd( config.get('exif_focal_sd', 0.01), config.get('radial_distorsion_k1_sd', 0.01), config.get('radial_distorsion_k2_sd', 0.01))
...
def bundle_single_view(graph, reconstruction, shot_id, config):
    ...
    for track_id in graph[shot_id]:
        if track_id in reconstruction.points:
            if config.get('tag_tracks',False) and graph[shot_id][track_id]['tag_feature']:
                if config.get('optimize_with_tag_tracks',False):
                    track = reconstruction.points[track_id]
                    x = track.coordinates
                    ba.add_point(str(track_id), x[0], x[1], x[2], True)
                    ba.add_tag_observation(str(shot_id), str(track_id), *graph[shot_id][track_id]['feature'])
                else:
                    track = reconstruction.points[track_id]
                    x = track.coordinates
                    ba.add_point(str(track_id), x[0], x[1], x[2], True)
    ...
    if config['bundle_use_gps']:
        g = shot.metadata.gps_position
        ba.add_position_prior(str(shot.id), g[0], g[1], g[2], shot.metadata.gps_dop)
...
def _pair_reconstructability_arguments(track_dict, config):
    ...
    return [(threshold, im1, im2, p1, p2) for (im1, im2), (tracks, p1, p2, ontag, tagid, cid) in track_dict.items()]
...
def reorder_commontracks_by_tag_connections(reconstruction, common_tracks, tag_graph):
    """Sort by number of connected tags to current reconstruction, then
    by number of common tracks"""
    # triplet that include number of tag connections
    common_tracks_with_tags = []

    # iterate through tracks, for each image, count number of connections to reconstruction
    for image, tracks in common_tracks:
        
        # count for number of connections image has to reconstruction
        tag_count = 0

        # if image has tags
        if tag_graph.has_node(image):

            # tags in image
            tags = tag_graph[image]

            # for each tag, find images connected
            for tag in tags:

                connected_images = tag_graph[tag]

                # for each connected image, see if it is in reconstruction
                for connected_image in connected_images:

                    # skip image of itself
                    if image == connected_image:
                        continue

                    # in reconstruction?
                    if connected_image in reconstruction.shots:

                        # increment tag connected count to reconstruction
                        tag_count += 1
        
        # store tag count for image
        common_tracks_with_tags.append( (image, tracks, tag_count) )

    # sort by tag_count, sort is stable, so common tracks will break ties
    common_tracks_with_tags = sorted(common_tracks_with_tags, key=lambda x: -x[2])
    
    # return in expect common tracks format (i.e. (image, num_tracks) )
    return [(x,y) for x,y,z in common_tracks_with_tags]
...
def resect(data, graph, reconstruction, shot_id):
    ...
    for track in graph[shot_id]:
        if track in reconstruction.points:
            #if tag feature and not optimizing with tag features, skip
            #if graph[track][shot_id]['tag_feature'] and not data.config.get('optimize_with_tag_tracks',False):
            #    continue
...
class TrackTriangulator:
    ...
    def triangulate(self, track, reproj_threshold, min_ray_angle_degrees):
        ...
        if len(os) >= 2:
            # tag feature, use different reproj_threshold and min_ray_angle_degrees
            #if self.graph[track][shot_id]['tag_feature']:
            #    e, X = csfm.triangulate_bearings_midpoint(os, bs, 10000, np.radians(0))
            #else:
            e, X = csfm.triangulate_bearings_midpoint(os, bs, reproj_threshold, np.radians(min_ray_angle_degrees))
            ...
            if X is not None:
                ...
                if self.graph[track][shot_id]['tag_feature']:
                    point.on_tag = True
                    point.tag_id = self.graph[track][shot_id]['tag_id']
                    point.tag_corner = self.graph[track][shot_id]['corner_id']
...
def remove_outliers(graph, reconstruction, config):
    ...
    if threshold > 0:
        ...
        for track in reconstruction.points:
            # do nothing about tag points, they aren't being used for optimization anyway
            #if reconstruction.points[track].on_tag and not config.get('optimize_with_tag_tracks',False):
            #    continue
...
def incremental_reconstruction(data):
    ...
    for im1, im2 in pairs:
        if im1 in remaining_images and im2 in remaining_images:
            tracks, p1, p2, _, _, _ = common_tracks[im1, im2]
...
#======================================================================================#
#============================== Reconstruction with Tags ==============================#
#======================================================================================#

#=============== Incremental Reconstruction With Tags ===============#
# starting point for incremental reconstruction when using tags
#
# input:
#    data - loaded data object with tracks and images
# output:
#    reconstructions - list of reconstructions from data
def incremental_reconstruction_with_tags(data):

    # start
    logger.info("Starting incremental recontsruction with tags")
    if not data.reference_lla_exists():
        data.invent_reference_lla()
    ...
    tags_graph = None
    tag_components = None
    if data.config.get('resection_with_tags',False):
        tags_graph = data.load_tags_graph()
        tag_components = matching.tag_connected_components(tags_graph)
    ...
    # order pairs by reconstructability
    pairs = compute_image_pairs_with_tags(common_tracks, data.config, tag_components)
    ...
    for im1, im2 in pairs:
        ...
        # if image pairs in remaining images, start with these two images
        if im1 in remaining_images and im2 in remaining_images:

            # tracks for images
            tracks, p1, p2, on_tag, tag_id, corner_id = common_tracks[im1, im2]

            # begin reconstruction
            reconstruction = bootstrap_reconstruction_with_tags(data, graph, im1, im2, p1, p2, on_tag)
            ...
            if reconstruction:
                ...
                # grow
                reconstruction = grow_reconstruction_with_tags(data, graph, reconstruction, remaining_images, gcp, tags_graph)
                reconstructions.append(reconstruction)

                # sort by number of registered images
                reconstructions = sorted(reconstructions, key = lambda x: -len(x.shots))

                # save reconstructions
                if data.config.get('experiments_path',False):
                    recon_name = os.path.join(data.config['experiments_path'],data.reconstruction_name_from_settings())
                    data.save_reconstruction(reconstructions,recon_name)
                else:
                    data.save_reconstruction(reconstructions)
...
#============= End Incremental Reconstruction With Tags =============#


#=============== Grow Reconstruction With Tags ===============#
# 
def grow_reconstruction_with_tags(data, graph, reconstruction, images, gcp, tags_graph):
    
    # bundle and align
    bundle(graph, reconstruction, None, data.config)
    align.align_reconstruction(reconstruction, gcp, data.config)

    # set should bundle and retriangulate
    should_bundle = ShouldBundle(data, reconstruction)
    should_retriangulate = ShouldRetriangulate(data, reconstruction)

    # iterate to add more images
    while True:

        # save partial reconstruction
        if data.config.get('save_partial_reconstructions', False):
            paint_reconstruction(data, graph, reconstruction)
        ...
        # sort tracks by most connected tags to current reconstruction, then common tracks
        if data.config.get('resection_with_tags',False):
            common_tracks = reorder_commontracks_by_tag_connections(reconstruction, common_tracks, tags_graph)
        
        # for each image, tracks in common tracks, ordered by most tracks to least
        logger.info("-------------------------------------------------------")
        for image, num_tracks in common_tracks:
            ...
            if resect(data, graph, reconstruction, image):
                ...
                # remove image from remaining
                logger.info("Adding {0} to the reconstruction".format(image))
                images.remove(image)

                # triangulate
                triangulate_shot_features(graph, reconstruction, image, data.config.get('triangulation_threshold', 0.004), data.config.get('triangulation_min_ray_angle', 2.0))
                ...
                # check for local bundle
                else:
                    if data.config['local_bundle_radius'] > 0:
                        bundle_local(graph, reconstruction, None, image, data.config)

                # should retriangulate
                if should_retriangulate.should(reconstruction):
                    logger.info("Re-triangulating")
                    retriangulate(graph, reconstruction, data.config)
                    bundle(graph, reconstruction, None, data.config)
    ...
    # return
    return reconstruction
#============= End Grow Reconstruction With Tags =============#



#=============== Bootstrap Reconstruction With Tags ===============#
# Starting point of a reconstruction. Finds the poses of two views
# and triangulates the features of these two views
#
# input:
#    data -
#    graph -
...
#    p2 -
# output:
#    reconstruction - initial two-view reconstruction
def bootstrap_reconstruction_with_tags(data, graph, im1, im2, p1, p2, ontag):

    # start: load pair info
    logger.info("Starting reconstruction with {} and {}".format(im1, im2))
    d1 = data.load_exif(im1)
    d2 = data.load_exif(im2)
    cameras = data.load_camera_models()
    ...
    # if we aren't optimizing with tag tracks, remove them from p1 and p2
    #if not data.config.get('optimize_with_tag_tracks',False):
    #    print 'boostrap reconstruction: remove tag tracks for optimization'
    #    p1 = p1[np.nonzero(np.array(ontag) == 0)[0],:]
    #    p2 = p2[np.nonzero(np.array(ontag) == 0)[0],:]
    logger.info("Common tracks: {}".format(len(p1)))

    # two view reconstruction
    thresh = data.config.get('five_point_algo_threshold', 0.006)
    min_inliers = data.config.get('five_point_algo_min_inliers', 50)
    R, t, inliers = two_view_reconstruction_with_tags(p1, p2, camera1, camera2, thresh)
    ...
    if len(inliers) > 5:
        ...
        # add first image to reconstruction
        shot1 = types.Shot()
        shot1.id = im1
        shot1.camera = cameras[str(d1['camera'])]
        shot1.pose = types.Pose()
        ...
        # add second image to reconstruction
        shot2 = types.Shot()
        shot2.id = im2
        shot2.camera = cameras[str(d2['camera'])]
        shot2.pose = types.Pose(R, t)
        ...
        triangulate_shot_features(graph, reconstruction, im1, data.config.get('triangulation_threshold', 0.004), data.config.get('triangulation_min_ray_angle', 2.0))
        logger.info("Triangulated: {}".format(len(reconstruction.points)))

        # check result
        if len(reconstruction.points) > min_inliers:
            
            # ba and retriangulate
            bundle_single_view(graph, reconstruction, im2, data.config)
            retriangulate(graph, reconstruction, data.config)
            bundle_single_view(graph, reconstruction, im2, data.config)
    ...
    # failure
    logger.info("Starting reconstruction with {} and {} failed")
#============= End Bootstrap Reconstruction With Tags =============#

#=============== Two View Reconstruction ===============#
def two_view_reconstruction_with_tags(p1, p2, camera1, camera2, threshold):
    """Reconstruct two views from point correspondences.
    ...
    # focal length 1.  Also, arctan(threshold) \approx threshold since
    # threshold is small
    T = run_relative_pose_ransac_with_tags( b1, b2, "STEWENIUS", 1 - np.cos(threshold), 1000)
    R = T[:, :3]
    t = T[:, 3]
    inliers = _two_view_reconstruction_inliers_with_tags(b1, b2, R, t, threshold)
    ...
    return cv2.Rodrigues(R.T)[0].ravel(), -R.T.dot(t), inliers
#============= End Two View Reconstruction =============#

#=============== Two View Reconstruction Inliers ===============#
def _two_view_reconstruction_inliers_with_tags(b1, b2, R, t, threshold):
    p = pyopengv.triangulation_triangulate(b1, b2, t, R)
    ...
    ok2 = np.linalg.norm(br2 - b2, axis=1) < threshold
    return np.nonzero(ok1 * ok2)[0]
#============= End Two View Reconstruction Inliers =============#

#=============== Run Relative Pose RANSAC ===============#
def run_relative_pose_ransac_with_tags(b1, b2, method, threshold, iterations):
    return pyopengv.relative_pose_ransac(b1, b2, method, threshold, iterations)
#============= End Run Relative Pose RANSAC =============#

#=============== Run Relative Pose Optimize Nonlinear ===============#
def run_relative_pose_optimize_nonlinear_with_tags(b1, b2, t, R):
    return pyopengv.relative_pose_optimize_nonlinear(b1, b2, t, R)
#============= End Run Relative Pose Optimize Nonlinear =============#


#=============== Compute Image Pairs With Tags ===============#
# returns all image pairs sorted by their reconstructability score
#
# input:
#    track_dict -
#    config -
# output:
#    list of pairs in sorted order by reconstructability
def compute_image_pairs_with_tags(track_dict, config, tag_components):

    # get slices
    args = _pair_reconstructability_arguments_with_tags(track_dict, config)
    
    # parallel if processe > 1
    processes = config.get('processes', 1)
    if processes == 1:
        result = map(_compute_pair_reconstructability_with_tags, args)
    else:
        p = Pool(processes)
        result = p.map(_compute_pair_reconstructability_with_tags, args)

    # filter out pairs with 0 scores
    pairs = [(im1, im2) for im1, im2, r in result if r > 0]

    # filter out scores <= 0 and sort scores
    score = [r for im1, im2, r in result if r > 0]
    order = np.argsort(-np.array(score))

    # sorted pairs
    sortedpairs = [pairs[o] for o in order]

    # sort scores by largest tag component, then score; i.e.,
    # all the pairs in the largest component come first, sorted by score; then
    # all the pairs in the second largest component come next, sorted by score;
    # lastly, pairs across components sorted by score
    if tag_components:
        
        # list of list to hold bins with pairs
        sorted_pair_bins = [[] for i in range(len(tag_components)+1)]

        # for each pair
        for sortedpair in sortedpairs:
            
            # for each tag component
            found = False
            for bct, tag_component in enumerate(tag_components):

                # if both images of the pair are in the tag_component
                if tag_component.graph.has_node(sortedpair[0]) and tag_component.graph.has_node(sortedpair[1]):
                    
                    # add to that tag components bin
                    sorted_pair_bins[bct].append(sortedpair)
                    found = True
                    break

            # pair is across components, add to last bin
            if not found:
                sorted_pair_bins[-1].append(sortedpair)

        # flatten bins to list
        sortedpairs = list(itertools.chain.from_iterable(sorted_pair_bins))

    # return pairs sorted by score
    return sortedpairs
#============= End Compute Image Pairs With Tags =============#

#=============== Pair Reconstructability Arguments With Tags ===============#
# creates slices for parallel calculation of reconstructability score
#
# input:
#    track_dict -
#    config -
# output:
#    list of images and features for reconstructability score calculation
def _pair_reconstructability_arguments_with_tags(track_dict, config):

    # get threshold from config
    threshold = config.get('homography_threshold', 0.004)
    
    # slice
    return [(threshold, im1, im2, p1, p2, ontag, tagid, cornerid, config.get('optimize_with_tag_tracks',False)) for (im1, im2), (tracks, p1, p2, ontag, tagid, cornerid) in track_dict.items()]
#============= End Pair Reconstructability Arguments With Tags =============#

#=============== Compute Pair Reconstructability With Tags ===============#
# wrapper for computing the reconstructability score 
#
# input:
#    args - from _pair_reconstructability_arguments_with_tags() method
# output:
#    tuple with image pair and score
def _compute_pair_reconstructability_with_tags(args):

    # args
    threshold, im1, im2, p1, p2, ontag, tagid, cornerid, optimize_with_tag_tracks = args

    # if we aren't optimizing with tag tracks, remove them from p1 and p2
    #if not optimize_with_tag_tracks:
    #    p1 = p1[np.nonzero(np.array(ontag) == 0)[0],:]
    #    p2 = p2[np.nonzero(np.array(ontag) == 0)[0],:]
    ...
    # score
    r = pairwise_reconstructability_with_tags(len(p1), inliers.sum())
    
    # return score
    return (im1, im2, r)
#============= End Compute Pair Reconstructability With Tags =============#

#=============== Pairwise reconstructability ===============#
# compute the reconstructability score
def pairwise_reconstructability_with_tags(common_tracks, homography_inliers):
    
    # get outlier ratio based on homography
    outliers = common_tracks - homography_inliers
    outlier_ratio = float(outliers) / common_tracks
    
    # if ratio better than 0.3, return common tracks
    if outlier_ratio > 0.3:
        return common_tracks
    # otherwise, return 0
    else:
        return 0
#============= End Pairwise reconstructability =============#
