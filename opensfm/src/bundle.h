...
struct TagPerspectiveReprojectionError 
{
  //constructor
  TagPerspectiveReprojectionError(double observed_x, double observed_y, double std_deviation)
      : observed_x_(observed_x)
      , observed_y_(observed_y)
      , scale_(1.0 / std_deviation)
  {}
  ...
  // overload () operator
  template <typename T>
  bool operator()(const T* const camera, const T* const shot, const T* const point, T* residuals) const
  {
    T camera_point[3];
    WorldToCameraCoordinates(shot, point, camera_point);
...
class BundleAdjuster {
 public:
  BundleAdjuster() {
    ...
    tag_loss_function_ = "TrivialLoss";
    tag_loss_function_threshold_ = 1;
    reprojection_error_sd_ = 1;
    tag_reprojection_error_sd_ = 1;
    tag_ortho_sd_ = 1;
    tag_size_sd_ = 1;
    focal_prior_sd_ = 1;
    k1_sd_ = 1;
    ...
    use_tag_size_constraint_ = false;
    use_tag_ortho_constraint_ = false;
    use_tag_reproj_constraint_ = false;
  ...
  void AddTag(double p1x, double p1y, double p1z,
              double p2x, double p2y, double p2z,
              double p3x, double p3y, double p3z,
              double p4x, double p4y, double p4z, double size)
  {
    BATag t;
    t.p1[0] = p1x; t.p1[1] = p1y; t.p1[2] = p1z;
    t.p2[0] = p2x; t.p2[1] = p2y; t.p2[2] = p2z;
    t.p3[0] = p3x; t.p3[1] = p3y; t.p3[2] = p3z;
    t.p4[0] = p4x; t.p4[1] = p4y; t.p4[2] = p4z;
    t.size = size;
    tags_.push_back(t);
  }
  ...
  void SetTagLossFunction(const std::string &function_name,
                       double threshold) {
    tag_loss_function_ = function_name;
    tag_loss_function_threshold_ = threshold;
  }

  void SetTagSizeSD(double tag_size_sd)
  {
    tag_size_sd_ = tag_size_sd;
  }

  void SetTagOrthoSD(double tag_ortho_sd)
  {
    tag_ortho_sd_ = tag_ortho_sd;
  }

  void SetTagReprojectionErrorSD(double tag_sd)
  {
    tag_reprojection_error_sd_ = tag_sd;
  }
  ...
  void SetUseTagSizeConstraint(bool v)
  {
    use_tag_size_constraint_ = v;
  }
  void SetUseTagOrthoConstraint(bool v)
  {
    use_tag_ortho_constraint_ = v;
  }
  void SetUseTagReprojConstraint(bool v)
  {
    use_tag_reproj_constraint_ = v;
  }
  ...
  void Run() {
    ...
    ceres::LossFunction *tag_loss;
    if (tag_loss_function_.compare("TruncatedLoss") == 0) {
      tag_loss = new TruncatedLoss(tag_loss_function_threshold_);
    } else if (tag_loss_function_.compare("TrivialLoss") == 0) {
      tag_loss = new ceres::TrivialLoss();
    } else if (tag_loss_function_.compare("HuberLoss") == 0) {
      tag_loss = new ceres::HuberLoss(tag_loss_function_threshold_);
    } else if (tag_loss_function_.compare("SoftLOneLoss") == 0) {
      tag_loss = new ceres::SoftLOneLoss(tag_loss_function_threshold_);
    } else if (tag_loss_function_.compare("CauchyLoss") == 0) {
      tag_loss = new ceres::CauchyLoss(tag_loss_function_threshold_);
    } else if (tag_loss_function_.compare("ArctanLoss") == 0) {
      tag_loss = new ceres::ArctanLoss(tag_loss_function_threshold_);
    }
    ...
    for (auto &observation : observations_) {
      switch (observation.camera->type()) {
        case BA_PERSPECTIVE_CAMERA:
        {
          ...
          if(use_tag_reproj_constraint_)
          {
            // tag point
            if(observation.on_tag)
            {
              if(observation.optimize)
              {
                // create cost function from TagPerspectiveReprojectionError and add to optimization
                ceres::CostFunction* cost_function = new ceres::AutoDiffCostFunction<TagPerspectiveReprojectionError, 2, 3, 6, 3>( new TagPerspectiveReprojectionError(observation.coordinates[0], observation.coordinates[1], tag_reprojection_error_sd_));            
                problem.AddResidualBlock(cost_function, tag_loss, c.parameters, observation.shot->parameters, observation.point->coordinates);
              }
              else
              {
                // do not optimize based on these observations
              }
            }
            // feature point
            else
            {
              // create cost function from PerspectiveReprojectionError and add to optimization
              ceres::CostFunction* cost_function = new ceres::AutoDiffCostFunction<PerspectiveReprojectionError, 2, 3, 6, 3>( new PerspectiveReprojectionError(observation.coordinates[0], observation.coordinates[1], reprojection_error_sd_));
              problem.AddResidualBlock(cost_function, loss, c.parameters, observation.shot->parameters, observation.point->coordinates);
            }
          }
          else
          {
    ...
    // Add tag size blocks
    if(use_tag_size_constraint_)
    {
      for (auto &tag : tags_)
      {
        // create cost function for tag size error
        ceres::CostFunction* cost_function = new ceres::AutoDiffCostFunction<TagSizeError, 1, 3, 3>(new TagSizeError(tag.size, tag_size_sd_));
        
        // add residual block for four tag edges (p1,p2), (p2,p3), (p3,p4), (p4,p1)
        problem.AddResidualBlock(cost_function, NULL, tag.p1, tag.p2);
        problem.AddResidualBlock(cost_function, NULL, tag.p2, tag.p3);
        problem.AddResidualBlock(cost_function, NULL, tag.p3, tag.p4);
        problem.AddResidualBlock(cost_function, NULL, tag.p4, tag.p1);
      }
    }

    // Add tag orthogonality blocks
    if(use_tag_ortho_constraint_)
    {
      for (auto &tag : tags_)
      {
        // create cost function for tag orthogonality error
        ceres::CostFunction* cost_function = new ceres::AutoDiffCostFunction<TagOrthogonalityError, 1, 3, 3, 3>(new TagOrthogonalityError(tag_ortho_sd_));

        // add residual block for four neighboring edge pairs (v12,v23), (v23,v34), (v34,v41), (v41,v12)
        problem.AddResidualBlock(cost_function, NULL, tag.p1, tag.p2, tag.p3);
        problem.AddResidualBlock(cost_function, NULL, tag.p2, tag.p3, tag.p4);
        problem.AddResidualBlock(cost_function, NULL, tag.p3, tag.p4, tag.p1);
        problem.AddResidualBlock(cost_function, NULL, tag.p4, tag.p1, tag.p2);
      }
    }
 ...
 private:
  ...
  std::vector<BATag> tags_;

  BAShot *unit_translation_shot_;

  std::string loss_function_;
  std::string tag_loss_function_;
  double loss_function_threshold_;
  double tag_loss_function_threshold_;
  double reprojection_error_sd_;
  double focal_prior_sd_;
  double k1_sd_;
  double k2_sd_;
  double tag_size_sd_;
  double tag_ortho_sd_;
  double tag_reprojection_error_sd_;
  bool compute_covariances_;
  bool covariance_estimation_valid_;
  bool compute_reprojection_errors_;
  bool use_tag_size_constraint_;
  bool use_tag_ortho_constraint_;
  bool use_tag_reproj_constraint_;
