...
class DataSet:
    ...
    def __tag_feature_path(self):
        """Return path of tag features directory"""
        return os.path.join(self.data_path, "tag_features")

    def __tag_feature_file(self, image):
        """
        Return path of tag feature file for specified image
        """
        return os.path.join(self.__tag_feature_path(), image + '.npz')
    ...
    def __save_tag_features(self, filepath, image, points, ids, idx, colors = None):
        io.mkdir_p(self.__tag_feature_path())
        np.savez_compressed(filepath, points=points.astype(np.float32), ids = ids, idx = idx, colors = colors)
    ...
    def load_tag_features(self, image):
        s = np.load(self.__tag_feature_file(image))
        return s['points'], s['ids'], s['idx'], s['colors']
    ...
    def save_tag_features(self, image, points, ids, idx, colors):
        self.__save_tag_features(self.__tag_feature_file(image), image, points, ids, idx, colors)
    ...
    def tag_matches_exists(self, image):
        return os.path.isfile(self.__tag_matches_file(image))
    ...
    def load_tag_matches(self, image):
        with gzip.open(self.__tag_matches_file(image), 'rb') as fin:
            matches = pickle.load(fin)
        return matches
    ...
    def __tags_graph_file(self, filename=None):
        """Return path of tag graph file"""
        return os.path.join(self.data_path, filename or 'tags_graph.csv')
    ...
    def load_tags_graph(self, filename=None):
        """Return graph (networkx data structure) of tags"""
        with open(self.__tags_graph_file(filename)) as fin:
            return load_tags_graph(fin)
    ...
    def save_tags_graph(self, graph, filename=None):
        with open(self.__tags_graph_file(filename), 'w') as fout:
            save_tags_graph(fout, graph)
    ...
    def __tag_detection_file(self, filename):
        """Return path of tag detection file"""
        return os.path.join(self.data_path, filename or 'tag_detections.json')

    def tag_detection_exists(self, filename=None):
        return os.path.isfile(self.__tag_detection_file(filename))

    def load_tag_detection(self, filename=None):
        with open(self.__tag_detection_file(filename)) as fin:
            tag_detections = io.tag_detections_from_json(json.load(fin))
    ...
    def reconstruction_name_from_settings(self):

        # get settings
        ba_size = self.config['ba_constraint_size']
        ba_ortho = self.config['ba_constraint_ortho']
        ba_reproj = self.config['ba_constraint_reproj']
        ba_size_sd = self.config['tag_size_sd']
        ba_ortho_sd = self.config['tag_ortho_sd']
        ba_reproj_sd = self.config['tag_reprojection_error_sd']
        tag_resection = self.config['resection_with_tags']
        align_method = self.config['align_method']
        keep_ratio = self.config['ratio_tags_to_keep']
        ...
        if tag_resection:
            name += 'TagResect_'
        if ba_size:
            name += 'BAS=' + str(ba_size_sd) + '_'
        if ba_ortho:
            name += 'BAO=' + str(ba_ortho_sd) + '_'
        if ba_reproj:
            name += 'BAR=' + str(ba_reproj_sd) + '_'
        if align_method == 'tag_scaling':
            name += 'TagScale'
        if keep_ratio != 1.0:
            name += 'Keep=' + str(keep_ratio) + '_'
        name += '.json'
...
def load_tracks_graph(fileobj):
    g = nx.Graph()
    for line in fileobj:
        image, track, observation, x, y, R, G, B, on_tag, tagid, cid = line.split('\t')
        ...
        g.add_edge(
            ...
            tag_feature=int(on_tag),
            tag_id=tagid,
            corner_id=int(cid))
    return g

def load_tags_graph(fileobj):
    g = nx.Graph()
    for line in fileobj:
        image, tagid = line.split('\t')
        g.add_node(image, bipartite=0)
        g.add_node(tagid, bipartite=1)
        g.add_edge(image, tagid)
    return g
    ...
    for node, data in graph.nodes(data=True):
        if data['bipartite'] == 0:
            ...
            for track, data in graph[image].items():
                ...
                tag_feature = data['tag_feature']
                tag_id = data['tag_id']
                corner_id = data['corner_id']
                fileobj.write('%s\t%s\t%d\t%g\t%g\t%g\t%g\t%g\t%d\t%s\t%d\n' % ( str(image), str(track), fid, x, y, r, g, b, tag_feature, tag_id, corner_id ))

def save_tags_graph(fileobj, graph):
    for node, data in graph.nodes(data=True):
        if data['bipartite'] == 0:
            image = node
            for tagid, data in graph[image].items():
                fileobj.write('%s\t%s\n' % ( str(image), str(tagid) ) )
